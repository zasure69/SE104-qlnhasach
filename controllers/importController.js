...
const createImportReceipt = async (req, res) => {
    // ... (Code cũ giữ nguyên logic tạo phiếu) ...
    try {
        ...
        if (inputDate > today) {
          return res.status(400).json({ error: "Ngày nhập không được sau ngày hôm nay" });
        }
    
        for (const item of chiTiet) {
          if (!item.SoLuong || item.SoLuong < 150) {
            return res.status(400).json({ error: "Số lượng nhập phải từ 150 trở lên" });
          }
    
          const tongNhap = (await db.CT_PNS.sum("SoLuong", { where: { MaSach: item.MaSach } })) || 0;
          const tongBan = (await db.CT_HD.sum("SoLuongBan", { where: { MaSach: item.MaSach } })) || 0;
          const soLuongTonHienTai = tongNhap - tongBan;
    
          if (soLuongTonHienTai >= 300) {
            const sach = await db.Sach.findByPk(item.MaSach, { include: [{ model: db.DauSach, attributes: ["TenSach"] }] });
            const tenSach = sach?.DauSach?.TenSach || item.MaSach;
            return res.status(400).json({ error: `Không thể nhập sách "${tenSach}" vì số lượng tồn hiện tại (${soLuongTonHienTai}) đã >= 300` });
          }
        }
    
        const tiLeGiaBan = await db.ThamSo.findOne({ where: { TenThamSo: "TiLeTinhDonGiaBan" } });
        ...
        await db.sequelize.transaction(async (t) => {
          ...
          tongTien = chiTiet.reduce((sum, item) => sum + item.SoLuong * item.DonGiaNhap, 0);
          ...
          for (const item of chiTiet) {
            ...
            const donGiaBan = item.DonGiaBan ? Math.round(parseFloat(item.DonGiaBan)) : Math.round(donGiaNhap * tiLe);
        ...
        return res.status(201).json({ message: "Tạo phiếu nhập sách thành công!", maPhieu: newMaPhieu, tongTien });
      } catch (err) {
...
const updateImportReceipt = async (req, res) => {
  try {
    ...
    if (NgayNhapPhieu) {
      ...
      today.setHours(0, 0, 0, 0);
      inputDate.setHours(0, 0, 0, 0);
      if (inputDate > today) return res.status(400).json({ error: "Ngày nhập không được sau ngày hôm nay" });
    }

    // --- LOGIC SỬA MỚI: CHECK TỒN KHO TRƯỚC KHI TRỪ ---
    // Kiểm tra xem việc xóa chi tiết cũ có làm âm kho không
    const oldDetailsToCheck = await db.CT_PNS.findAll({ where: { MaPhieuNhap: maPhieu } });
    ...
    // Logic kiểm tra nhập mới (như cũ)
    for (const item of chiTiet) {
      if (!item.SoLuong || item.SoLuong < 150) return res.status(400).json({ error: "Số lượng nhập phải từ 150 trở lên" });
      if (!item.DonGiaNhap || item.DonGiaNhap < 0) return res.status(400).json({ error: "Đơn giá nhập không hợp lệ" });

      const oldDetail = await db.CT_PNS.findOne({ where: { MaPhieuNhap: maPhieu, MaSach: item.MaSach } });
      const tongNhap = (await db.CT_PNS.sum("SoLuong", { where: { MaSach: item.MaSach } })) || 0;
      const tongBan = (await db.CT_HD.sum("SoLuongBan", { where: { MaSach: item.MaSach } })) || 0;
      ...
      if (soLuongTonTruocCapNhat >= 300) {
        const sach = await db.Sach.findByPk(item.MaSach, { include: [{ model: db.DauSach, attributes: ["TenSach"] }] });
        const tenSach = sach?.DauSach?.TenSach || item.MaSach;
        return res.status(400).json({ error: `Không thể nhập sách "${tenSach}" vì số lượng tồn hiện tại (${soLuongTonTruocCapNhat}) đã >= 300` });
      }
    }
    ...
    await db.sequelize.transaction(async (t) => {
      ...
      const oldDetails = await db.CT_PNS.findAll({ where: { MaPhieuNhap: maPhieu }, transaction: t });
      for (const old of oldDetails) {
        const sach = await db.Sach.findByPk(old.MaSach, { transaction: t });
        if (sach) {
          await sach.update({ SoLuongTon: sach.SoLuongTon - old.SoLuong }, { transaction: t });
        }
      }
      await db.CT_PNS.destroy({ where: { MaPhieuNhap: maPhieu }, transaction: t });

      // 2. Cập nhật phiếu nhập
      const tongTien = chiTiet.reduce((sum, item) => sum + item.SoLuong * item.DonGiaNhap, 0);
      await receipt.update({
        NgayNhapPhieu: NgayNhapPhieu || receipt.NgayNhapPhieu,
        TongTien: tongTien,
        MaNhanVien: MaNhanVien !== undefined ? MaNhanVien : receipt.MaNhanVien,
      }, { transaction: t });
      ...
      for (const item of chiTiet) {
        ...
        const sach = await db.Sach.findByPk(item.MaSach, { transaction: t });
        if (sach) {
          await sach.update({ SoLuongTon: sach.SoLuongTon + soLuong, DonGia: donGiaBan }, { transaction: t });
        }
      }
    });
...
const deleteImportReceipt = async (req, res) => {
  try {
    ...
    const detailsToCheck = await db.CT_PNS.findAll({ where: { MaPhieuNhap: maPhieu } });
