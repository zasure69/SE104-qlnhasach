 ...
 */
const { sequelize, Quyen, VaiTro, VaiTro_Quyen, NhanVien } = require('../models');
...
const defaultQuyens = [
  ...
  // Sách
  { MaQuyen: 'sach.xem', TenQuyen: 'Xem danh sách sách', MoTa: 'Xem thông tin sách', NhomQuyen: 'sach' },
  { MaQuyen: 'sach.them', TenQuyen: 'Thêm sách mới', MoTa: 'Thêm đầu sách và sách mới', NhomQuyen: 'sach' },
  { MaQuyen: 'sach.sua', TenQuyen: 'Sửa thông tin sách', MoTa: 'Chỉnh sửa thông tin sách', NhomQuyen: 'sach' },
  { MaQuyen: 'sach.xoa', TenQuyen: 'Xóa sách', MoTa: 'Xóa sách khỏi hệ thống', NhomQuyen: 'sach' },
...
async function seedPermissions() {
  ...
  try {
    ...
    for (const vaiTroData of defaultVaiTros) {
      ...
      console.log(`  ✓ ${vaiTroData.TenVaiTro}: ${vaiTroData.Quyens.length} quyền`);
    }
    ...
    if (adminVaiTro) {
      ...
      for (const admin of adminUsers) {
        if (!admin.MaVaiTro) {
          await admin.update({ MaVaiTro: adminVaiTro.MaVaiTro }, { transaction: t });
